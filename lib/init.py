class pathsBib: 
    ...
    def __init__(self, config):
        ...
        self.model_id = config['model'] + '_t' + str(config['params']['time_lag']) +  '_l' + str(config['params']['num_layers']) + '_d' + str(config['params']['d_model']) + '_h' + str(config['params']['nhead']) + '_train' + str(config['train']['train_split']) + '_test' + str(config['train']['test_split']) + '_Ntrain' + str(config['train']['sample_train']) + '_Ntest' + str(config['train']['sample_test'])
        self.model_id = self.model_id.replace('.', '_')
        self.model_dir = self.latent_dir + self.model_id + '/'
        self.log_path = self.model_dir + 'out.log'
